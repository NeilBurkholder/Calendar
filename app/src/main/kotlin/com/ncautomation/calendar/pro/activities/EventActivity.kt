...
import com.ncautomation.calendar.pro.adapters.AutoCompleteTextViewAdapter
import com.ncautomation.calendar.pro.databinding.ActivityEventBinding
import com.ncautomation.calendar.pro.databinding.ItemAttendeeBinding
import com.ncautomation.calendar.pro.dialogs.*
...
import com.ncautomation.commons.dialogs.ConfirmationAdvancedDialog
...
import com.ncautomation.commons.views.MyAutoCompleteTextView
import org.joda.time.DateTime
import org.joda.time.DateTimeZone
import java.util.TimeZone
import java.util.regex.Pattern

class EventActivity : com.ncautomation.calendar.pro.activities.SimpleActivity() {
    ...
    override fun onBackPressed() {
        if (System.currentTimeMillis() - mLastSavePromptTS > SAVE_DISCARD_PROMPT_INTERVAL && isEventChanged()) {
            ...
            ConfirmationAdvancedDialog(
                activity = this,
                message = "",
                messageId = com.ncautomation.commons.R.string.save_before_closing,
                positive = com.ncautomation.commons.R.string.save,
                negative = com.ncautomation.commons.R.string.discard
            ) {
                if (it) {
                    saveCurrentEvent()
                } else {
    ...
    private fun getRepeatXthDayInMonthString(includeBase: Boolean, repeatRule: Int): String {
        ...
        val monthString = resources.getStringArray(com.ncautomation.commons.R.array.in_months)[mEventStartDateTime.monthOfYear - 1]
        ...
        when {
            mRepeatInterval.isXWeeklyRepetition() -> {
                binding.eventRepetitionRule.text = if (mRepeatRule == EVERY_DAY_BIT) {
                    getString(com.ncautomation.commons.R.string.every_day)
                } else {
    ...
    private fun showReminderTypePicker(currentValue: Int, callback: (Int) -> Unit) {
        val items = arrayListOf(
            RadioItem(REMINDER_NOTIFICATION, getString(com.ncautomation.commons.R.string.notification)),
            RadioItem(REMINDER_EMAIL, getString(com.ncautomation.commons.R.string.email))
        )
    ...
    private fun updateReminderTypeImage(view: ImageView, reminder: Reminder) {
        ...
        val drawable = if (reminder.type == REMINDER_NOTIFICATION) {
            com.ncautomation.commons.R.drawable.ic_bell_vector
        } else {
            com.ncautomation.commons.R.drawable.ic_mail_vector
        }
    ...
    private fun updateCurrentCalendarInfo(currentCalendar: CalDAVCalendar?) = binding.apply {
        ...
        if (currentCalendar == null) {
            ...
            val mediumMargin = resources.getDimension(com.ncautomation.commons.R.dimen.medium_margin).toInt()
            ...
            ensureBackgroundThread {
                ...
                runOnUiThread {
                    eventCaldavCalendarName.apply {
                        ...
                        setPadding(paddingLeft, paddingTop, paddingRight, resources.getDimension(com.ncautomation.commons.R.dimen.tiny_margin).toInt())
                    }
    ...
    private fun duplicateEvent() {
        ...
        hideKeyboard()
        finish()
        Intent(this, com.ncautomation.calendar.pro.activities.EventActivity::class.java).apply {
        ...
        if (mEvent.getReminders().isNotEmpty()) {
            handleNotificationPermission { granted ->
                if (granted) {
                    ...
                    PermissionRequiredDialog(this, com.ncautomation.commons.R.string.allow_notifications_reminders, { openNotificationSettings() })
                }
            }
        } else {
            storeEvent(wasRepeatable)
    ...
    private fun setupTimeZone() {
        hideKeyboard()
        Intent(this, com.ncautomation.calendar.pro.activities.SelectTimeZoneActivity::class.java).apply {
