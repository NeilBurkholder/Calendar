...
import com.ncautomation.calendar.pro.databinding.ActivitySettingsBinding
...
class SettingsActivity : SimpleActivity() {
    ...
    private fun setupStartWeekOn() = binding.apply {
        val items = arrayListOf(
            RadioItem(DateTimeConstants.SUNDAY, getString(com.ncautomation.commons.R.string.sunday)),
            RadioItem(DateTimeConstants.MONDAY, getString(com.ncautomation.commons.R.string.monday)),
            RadioItem(DateTimeConstants.TUESDAY, getString(com.ncautomation.commons.R.string.tuesday)),
            RadioItem(DateTimeConstants.WEDNESDAY, getString(com.ncautomation.commons.R.string.wednesday)),
            RadioItem(DateTimeConstants.THURSDAY, getString(com.ncautomation.commons.R.string.thursday)),
            RadioItem(DateTimeConstants.FRIDAY, getString(com.ncautomation.commons.R.string.friday)),
            RadioItem(DateTimeConstants.SATURDAY, getString(com.ncautomation.commons.R.string.saturday)),
        )
    ...
    private fun getDisplayPastEventsText(displayPastEvents: Int): String {
        return if (displayPastEvents == 0) {
            getString(com.ncautomation.commons.R.string.never)
        } else {
    ...
    private fun updateDefaultDurationText() {
        ...
        binding.settingsDefaultDuration.text = if (duration == 0) {
            "0 ${getString(com.ncautomation.commons.R.string.minutes_raw)}"
        } else {
    ...
    private fun parseFile(inputStream: InputStream?) {
        ...
        runOnUiThread {
            val msg = if (configValues.size > 0) {
                com.ncautomation.commons.R.string.settings_imported_successfully
            } else {
                com.ncautomation.commons.R.string.no_entries_for_importing
            }
        ...
        ensureBackgroundThread {
            ...
            if (events.isEmpty()) {
                ...
                IcsExporter(this).exportEvents(outputStream, events, true) { result ->
                    toast(
                        when (result) {
                            IcsExporter.ExportResult.EXPORT_OK -> com.ncautomation.commons.R.string.exporting_successful
                            IcsExporter.ExportResult.EXPORT_PARTIAL -> com.ncautomation.commons.R.string.exporting_some_entries_failed
                            else -> com.ncautomation.commons.R.string.exporting_failed
                        }
                    )
                }
            }
