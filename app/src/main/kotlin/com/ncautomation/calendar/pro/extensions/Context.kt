...
import com.ncautomation.calendar.pro.activities.EventActivity
import com.ncautomation.calendar.pro.activities.EventTypePickerActivity
import com.ncautomation.calendar.pro.activities.SnoozeReminderActivity
import com.ncautomation.calendar.pro.activities.TaskActivity
import com.ncautomation.calendar.pro.databases.EventsDatabase
import com.ncautomation.calendar.pro.databinding.DayMonthlyEventViewBinding
import com.ncautomation.calendar.pro.helpers.*
import com.ncautomation.calendar.pro.helpers.Formatter
import com.ncautomation.calendar.pro.interfaces.EventTypesDao
import com.ncautomation.calendar.pro.interfaces.EventsDao
import com.ncautomation.calendar.pro.interfaces.TasksDao
import com.ncautomation.calendar.pro.interfaces.WidgetsDao
import com.ncautomation.calendar.pro.models.*
import com.ncautomation.calendar.pro.receivers.AutomaticBackupReceiver
import com.ncautomation.calendar.pro.receivers.CalDAVSyncReceiver
import com.ncautomation.calendar.pro.receivers.NotificationReceiver
import com.ncautomation.calendar.pro.services.MarkCompletedService
import com.ncautomation.calendar.pro.services.SnoozeService
...
fun Context.scheduleNextEventReminder(event: Event, showToasts: Boolean) {
    ...
    if (validReminders.isEmpty()) {
        if (showToasts) {
            toast(com.ncautomation.commons.R.string.saving)
        }
        return
    }
...
fun Context.scheduleEventIn(notifyAtMillis: Long, event: Event, showToasts: Boolean) {
    ...
    if (showToasts) {
        ...
        val msg = String.format(getString(com.ncautomation.commons.R.string.time_remaining), formatSecondsToTimeString(secondsTillNotification.toInt()))
        toast(msg)
    }
...
fun Context.backupEventsAndTasks() {
    ...
    ensureBackgroundThread {
        ...
        IcsExporter(this).exportEvents(outputStream, events, showExportingToast = false) { result ->
            when (result) {
                IcsExporter.ExportResult.EXPORT_PARTIAL -> toast(com.ncautomation.commons.R.string.exporting_some_entries_failed)
                IcsExporter.ExportResult.EXPORT_FAIL -> toast(com.ncautomation.commons.R.string.exporting_failed)
                else -> {}
            }
...
fun Context.getRepetitionText(seconds: Int) = when (seconds) {
    ...
    YEAR -> getString(R.string.yearly)
        ...
        when {
            seconds % YEAR == 0 -> resources.getQuantityString(com.ncautomation.commons.R.plurals.years, seconds / YEAR, seconds / YEAR)
            seconds % MONTH == 0 -> resources.getQuantityString(com.ncautomation.commons.R.plurals.months, seconds / MONTH, seconds / MONTH)
            seconds % WEEK == 0 -> resources.getQuantityString(com.ncautomation.commons.R.plurals.weeks, seconds / WEEK, seconds / WEEK)
            else -> resources.getQuantityString(com.ncautomation.commons.R.plurals.days, seconds / DAY, seconds / DAY)
        }
    }
}
...
fun Context.notifyEvent(originalEvent: Event) {
    ...
    val displayedStartDate = when (startDate) {
        LocalDate.now() -> ""
        LocalDate.now().plusDays(1) -> getString(com.ncautomation.commons.R.string.tomorrow)
...
fun Context.getNotification(pendingIntent: PendingIntent, event: Event, content: String, publicVersion: Boolean = false): Notification? {
    ...
    val builder = NotificationCompat.Builder(this, channelId)
        ...
        .apply {
            ...
            addAction(
                com.ncautomation.commons.R.drawable.ic_snooze_vector,
                getString(com.ncautomation.commons.R.string.snooze),
...
fun Context.launchNewEventIntent(dayCode: String = Formatter.getTodayCode(), allowChangingDay: Boolean = false) {
    Intent(applicationContext, com.ncautomation.calendar.pro.activities.EventActivity::class.java).apply {
...
fun Context.getDayOfWeekString(dayOfWeek: Int): String {
    val dayOfWeekResId = when (dayOfWeek) {
        DateTimeConstants.MONDAY -> com.ncautomation.commons.R.string.monday
        DateTimeConstants.TUESDAY -> com.ncautomation.commons.R.string.tuesday
        DateTimeConstants.WEDNESDAY -> com.ncautomation.commons.R.string.wednesday
        DateTimeConstants.THURSDAY -> com.ncautomation.commons.R.string.thursday
        DateTimeConstants.FRIDAY -> com.ncautomation.commons.R.string.friday
        DateTimeConstants.SATURDAY -> com.ncautomation.commons.R.string.saturday
        DateTimeConstants.SUNDAY -> com.ncautomation.commons.R.string.sunday
...
fun Context.getShortDaysFromBitmask(bitMask: Int): String {
    ...
    val weekDays = withFirstDayOfWeekToFront(resources.getStringArray(com.ncautomation.commons.R.array.week_days_short).toList())
